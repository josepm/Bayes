...
def _change_data(n_pts, type_, n_cpt=1, noise_level=0.0, slow_bleed=0.0):
...
def change_data(n_pts, additive=True, seasonalities=None, trend_cpt=-1, level_cpt=-1, noise_level=0.0, slow_bleed=0.0):
    # generate data for trend changes
    # t: number of points
    ...
    trend_ = _change_data(n_pts, 'trend', n_cpt=trend_cpt, noise_level=noise_level, slow_bleed=slow_bleed) if trend_cpt >= 0 else [n_val, None, None]
    level_ = _change_data(n_pts, 'level', n_cpt=level_cpt, noise_level=noise_level, slow_bleed=slow_bleed) if level_cpt >= 0 else [n_val, None, None]
...
def set_breaks(n_pts, tbreak_, theta, slow_bleed):
    ...
    thetat_[tbreak_[n_cpt - 1]:] = theta[n_cpt] * np.array([(1 + slow_bleed) ** n for n in range(tbreak_[-1] - tbreak_[n_cpt - 1])])
